...
class PyCanvasGrader:
    ...
    def assignments(self, course_id: int, ungraded: bool = True) -> list:
        ...
        """
        url = (
            "https://sit.instructure.com/api/v1/courses/"
            + str(course_id)
    ...
    def user(self, course_id: int, user_id: int) -> dict:
        ...
        """
        url = "https://sit.instructure.com/api/v1/courses/%i/users/%i" % (
            course_id,
            user_id,
        )
    ...
    def grade_submission(self, course_id, assignment_id, user_id, grade):
        ...
        url = (
            "https://sit.instructure.com/api/v1/courses/%i/assignments/%i/submissions/%i/?submission[posted_grade]=%i"
    ...
    def from_file(cls, filename):
        with open("skeletons/" + filename) as skeleton_file:
            try:
                ...
                print(
                    "There is an error in the %s skeleton file. This skeleton will not be available"
                    % filename
                )
                return None
            try:
...
class AssignmentTest:
    ...
    def __init__(
        ...
        output_match: str = None,
        output_regex: str = None,
        numeric_match: list = None,
        negate_match: bool = False,
        exact_match: bool = False,
        timeout: int = None,
        fail_notif: dict = None,
        point_val: int = 0,
    ):
        """
    ...
    def from_json_dict(cls, json_dict: dict):
        try:
            ...
            vars_dict = {
                ...
                "ask_for_target": ask_for_target,
                "include_filetype": include_filetype,
    ...
    def target_prompt(cls, command: str):
        ...
        if len(os.listdir(os.getcwd())) < 1:
            print(
                'This directory is empty, unable to choose a file for "%s" command'
                % command
            )
            return None
        ...
        for count, file_name in enumerate(os.listdir(os.getcwd())):
            ...
            print(
                "%i.\t%s" % (count - sub + 1, file_name)
            )  # The minus 1 is to hide the 0-based numbering
    ...
    try:
        if os.path.exists("temp"):
            if os.path.exists("old-temp"):
                shutil.rmtree(
                    os.path.join(os.path.dirname(os.path.abspath(__file__)), "old-temp")
                )
            os.rename("temp", "old-temp")
        os.makedirs("temp", exist_ok=True)
    except BaseException:
        print(
            'An error occurred while initializing the "temp" directory. Please delete/create the directory manually and re-run the program'
        )
        exit(1)
...
def main():
    ...
    course_choice = (
        choose_val(len(course_list)) - 1
    )  # the minus 1 is to hide the 0-based numbering

    print("Show only ungraded assignments? (y or n):")
    ungraded = choose_bool()
    course_id = course_list[course_choice].get("id")
    assignment_list = grader.assignments(
        course_list[course_choice].get("id"), ungraded=ungraded
    )
    ...
    for submission in submission_list:
        if (
            ungraded_only and submission.get("grader_id") is not None
        ):  # Skip assignments that have been graded already
            continue
        user_id = submission.get("user_id")
        if submission.get("attachments") is not None:
            if grader.download_submission(
                submission, os.path.join("temp", str(user_id))
            ):
                user_submission_dict[user_id] = submission["id"]
            else:
                print(
                    "There was a problem downloading this user's submission. Skipping."
                )
    ...
    if len(user_submission_dict) > 1:
        s = "s"
    print(
        "Successfully retrieved %i submission%s. Is this correct? (y or n):"
        % (len(user_submission_dict), s)
    )
    correct = choose_bool()
    if not correct:
        restart_program(grader)
    ...
    if len(skeleton_list) < 1:
        print(
            "Could not find any skeleton files in the skeletons directory. Would you like to create one now? (y or n):"
        )
        if choose_bool():
            print("unimplemented")
        else:
            ...
            if not grade_conf:
                ...
                if selected_action == "Submit this grade":
                    grader.grade_submission(
                        course_id, assignment_id, cur_user_id, score
                    )
                    print("Grade submitted")
                    break
